...
//    CoordManager CM(config_str);
    ...
    cv::Point2f* regA_point = (cv::Point2f*)malloc(sizeof(cv::Point2f)*reg_point_size);
    regA_point[0] = cv::Point2f(100.0f, 100.0f);
    regA_point[1] = cv::Point2f(120.0f, 200.0f);
    regA_point[2] = cv::Point2f(80.0f, 400.0f);
    regA_point[3] = cv::Point2f(20.0f, 287.0f);
    CV_Reg->regInfo[0].count = reg_point_size;
    CV_Reg->regInfo[0].point = regA_point;

    cv::Point2f* regB_point = (cv::Point2f*)malloc(sizeof(cv::Point2f)*reg_point_size);
    regB_point[0] = cv::Point2f(326.36f, 64.45f);
    regB_point[1] = cv::Point2f(339.16f, 128.24f);
    regB_point[2] = cv::Point2f(313.73f, 255.86f);
    regB_point[3] = cv::Point2f(275.41f, 183.80f);
    ...
    A_Human->humanInfo[0].point = cv::Point2f(100.0f, 100.0f);
    A_Human->humanInfo[1].human_id = 2;
    A_Human->humanInfo[1].point = cv::Point2f(30.0f, 110.0f);
    A_Human->humanInfo[2].human_id = 3;
    A_Human->humanInfo[2].point = cv::Point2f(35.0f, 100.0f);
    ...
    B_Human->humanInfo[0].point = cv::Point2f(326.36f, 64.45f);
    B_Human->humanInfo[1].human_id = 5;
    B_Human->humanInfo[1].point = cv::Point2f(284.9f, 64.49f);
