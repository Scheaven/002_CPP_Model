...
struct Point
{
    ...
    Point(double a = , double b = , double c = ) { x = a; y = b; z = c; } // 构造函数
};
// 点的加法
...
double distance(const Point& p1, const Point& p2)
{
    return(sqrt(pow(p1.x - p2.x, ) + pow(p1.y - p2.y, ) + pow(p1.z - p2.z, )));
}
...
double length(const Point& vec)
{
    return (sqrt(pow(vec.x, ) + pow(vec.y, ) + pow(vec.z, )));
}
...
double ptolDistance(const Point& p, const Line& l)
{
    ...
    // 根据勾股定理计算点的距离
    double distance = sqrt(pow(length(line_vec), ) - pow(project_len, ));
...
Point ptolSymmetry(const Point& p, const Line& l)
{
    ...
    // 对称点
    Point symmetry_p = add(p, mul(project_vec, ));
...
double Sin(const Point& vec1, const Point& vec2)
{
    return sqrt(1.0 - pow(Cos(vec1, vec2), ));
}
...
vector<Point> firstOrderBezier(const Point& s, const Point& e, int inter_num)
{
    vector<Point> res;
    res.push_back(s);
    for (int i = ; i <= inter_num; ++i)
    {
        double a1 = double(i) / double(inter_num + );
        double a2 = 1.0 - a1;
...
vector<Point> secondOrderBezier(const Point& s, const Point& e, const Point& p, int inter_num)
{
    ...
    for (int i = ; i <= inter_num; ++i)
    {
        double a = double(i) / double(inter_num + );
        double a1 = pow(a,);
        double a2 =  * a * (1.0 - a);
        double a3 = pow(1.0 - a, );
...
vector<Point> thirdOrderBezier(const Point& s, const Point& e, const Point& p1, const Point& p2, int inter_num)
{
    ...
    for (int i = ; i <= inter_num; ++i)
    {
        double a = double(i) / double(inter_num + );
        double a1 = pow(a, );
        double a2 =  * pow(a, ) * (1.0 - a);
        double a3 =  * pow(1.0 - a, ) * a;
        double a4 = pow(1.0 - a, );
...
void checkConvex(const vector<Point>& polygon, vector<bool>& flags)
{
    ...
    */
    ...
    while (size)
    {
        if (multiply(polygon[index], polygon[(index + ) % size], polygon[(index + ) % size]).z >= )
        {
            flags[(index + ) % size] = true;
        }
        else
        {
            flags[(index + ) % size] = false;
        }
        index++;
        size--;
    }
...
double areaOfPolygon(const vector<Point>& polygon)
{
    ...
    for (int i = ; i < size; ++i)
    {
        area += polygon[i].y * (polygon[(i -  + size) % size].x - polygon[(i + ) % size].x);
    }
...
bool isPointInPolygon(const vector<Point>& polygon, const Point& p)
{
    ...
    {
        Point s = polygon[i];
        Point e = polygon[(i + ) % polygon.size()];
        Line seg(s, e, true);
        Point inter_p;
...
bool isCircleInPolygon(const vector<Point>& polygon, const Point& c, double radius)
{
    for (int i = ; i < polygon.size(); ++i)
    {
        ...
        const Point& p2 = polygon[(i + ) % polygon.size()];
        Line line(p1, p2, true);
        if (segToCircle(c, radius, line) != )
            return false;
    }
    return true;
}
...
vector<Point> findConvex(const vector<Point>& points)
{
    ...
    // 存入第一个和第二个点
    upper_hull.push_back(tmp_points[]);
    upper_hull.push_back(tmp_points[]);
    for (int i = ; i < tmp_points.size(); ++i)
    {
        upper_hull.push_back(tmp_points[i]);
        while (upper_hull.size() >  && multiply(sub(upper_hull[upper_hull.size() - ], upper_hull[upper_hull.size() - ]), sub(upper_hull[upper_hull.size() - ], upper_hull[upper_hull.size() - ])).z >= )
        {
    ...
    // 存入倒数第一第二个点
    lower_hull.push_back(tmp_points[tmp_points.size() - ]);
    lower_hull.push_back(tmp_points[tmp_points.size() - ]);
    for (int i = tmp_points.size() - ; i >= ; --i)
    {
        lower_hull.push_back(tmp_points[i]);
        while (lower_hull.size() >  && multiply(sub(lower_hull[lower_hull.size() - ], lower_hull[lower_hull.size() - ]), sub(lower_hull[lower_hull.size() - ], lower_hull[lower_hull.size() - ])).z >= )
        {
...
Point centerOfPolygon(const vector<Point>& polygon)
{
    ...
    for (int i = ; i < polygon.size(); ++i)
    {
        ...
        Point next_p = polygon[(i + ) % polygon.size()];
        Triangle t(origin, curr_p, next_p);
 
        double curr_area = areaOfTriangle(t);
        polygon_area += curr_area;
 
        center = add(center, mul(div(add(curr_p, next_p), ), curr_area));
    }
...
Point pointInPolygon(const vector<Point>& polygon)
{
    ...
    // 获取凸顶点前后一个点
    int size = polygon.size();
    Point pre_p = polygon[(index -  + size) % size];
    Point next_p = polygon[(index + ) % size];
    ...
    for (int i = ; i < polygon.size(); ++i)
    {
        if (i == index || i == ((index -  + size) % size) || i == ((index + ) % size))
            continue;
        flag = true;
    ...
    if (!flag)
    {
        return div(add(pre_p, next_p), );
    }
    // 返回最近点和凸顶点的中点
    return div(add(convex_p, min_p), );
}
